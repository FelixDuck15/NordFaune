...
\subsection{Protection contre les conditions climatiques}
\label{conditions_climatiques}
Situé dans le grand nord, le système devra faire face à des périodes de froids extrêmes. En effet, dans la zone de déploiement, les températures varient de –20 à +20°C. Donc, il faut s’assurer que le système maintienne son fonctionnement malgré la température auquel il va faire face. La pondération de la section est définie à 10\%. Une valeur de 0 à 1 est attribuée si l’isolation est excellente, une valeur de 7.5\% si l’isolation est passable. Tout fois si l’isolation est faible le système devra être refusée. 

\subsection{Difficulté d’installation et de déploiement}
\label{installation_deploiement}
À la demande du client, le système devra pouvoir être installé par deux personnes ne disposant d'aucune compétence en génie. De plus, la taille minimale du système devra être d'un mètre cube. Puisque qu'une installation et qu'un déploiement facile font partie des volontés de l'employeur, une pondération de 8\% sera attribuée à ce critère. Dans le but de faciliter son installation, la masse ainsi que la taille du système devront, dans la mesure du possible, être réduites au minimum. C'est pourquoi le barème de cette section sera décrit par la fonction suivante:
...
Le système pourrait être recouvert de neige sur une période pouvant allée jusqu'à huit mois. Il est donc important de protéger les équipements électroniques du poids de la neige afin d'assurer leur bon fonctionnent tout au long de la période de collecte de données. C'est pourquoi une pondération de 12\% sera attribuée pour ce critère.
\bigskip
...
Afin d’assurer l’exploitation scientifique du système, les données recueillies devront être archivées de façon à faciliter leur compilation à des fins statistiques. Les données recueillies devront également respecter certains critères de qualité. Les vidéos devront avoir une résolution minimum de 720P, 15 FPS. Elles devront être d’une durée minimale de 5 secondes et il devra s’écouler un temps maximal de 1h entre celles-ci. Pour assurer l’exploitation scientifique des données, il sera nécessaire d’optimiser la quantité de données recueillies par rapport à leur qualité. Finalement, une pondération de 5\% sera accordée à ce critère puisque le critère \ref{detection_mammiferes} se démarque clairement des autres par son importance étant donné qu’il touche à l’objectif principal du projet. Le barème est donné par l’équation suivante où f est la fréquence d’images par seconde des vidéos. On établit que 60 FPS (résolution considérée fluide) est l’idéal et 15 FPS est le minimum. Tout système pour lequel cette équation donne un résultat inférieur à 0 ne sera pas retenu:   
...
\begin{table}[htp]
    ...
    \begin{tabular}{|l|c|c|c|c|}
        ...
        \ref{resistance_neige} Protection contre les conditions climatiques & 10\% & Voir barème \ref{resistance_neige} & 0 & 100 \\
        \ref{installation_deploiement} Difficulté d’installation et de déploiement & 8\% & $\frac{G-1}{M}$ & 0 & \\
        \ref{resistance_neige} Résistance aux effets de l'enfouissement sous la neige & 12\% & $\sqrt{x}$ & 10 & \\
        ...
        \ref{conservation_donnees} Pérennité du stockage des données & 5\% & $\frac{d-12}{12}$ & 12 & \\
